...
class DES:
    def __init__(self, *args):
        self.key = args[1]
        self.plaintext = args[0]

    def IPChange(self, text):
        """
        IP转换
        :return:
        """
    ...
    def createKey(self):
        ...
        # 16轮置换
        for i in range(0, 16):
            L0 = self.move(L0, MoveTimes[i])
            R0 = self.move(R0, MoveTimes[i])
            # 合并
            mergeKey = L0 + R0

            tempKey = ""
            for n in COMPRESS_MATRIXS:
                tempKey += mergeKey[n - 1]
            subKey.append(tempKey)
        return subKey

    def move(self, text, number):
        """
        循环移位
        :return:
        """
    ...
    def main(self):
        ...
        for subKey in subKeyList:
            ...
            Rn_expend = self.E_expend(Rn)
            S_Input = int(Rn_expend, base=2) ^ int(subKey, base=2)
            subStr = self.S_sub(S_Input)

            (Ln, Rn) = self.P(Ln, subStr, Rn)

        (Ln, Rn) = (Rn, Ln)
        en_text = self.IP_reverse(Ln, Rn)
        return en_text
