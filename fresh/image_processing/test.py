...
def show_data(new_year, pre_year, data, s_pre_double, s_pre_triple):
    ...
    pyplot.figure(figsize=(14, 6), dpi=80)  # 设置绘图区域的大小和像素
    pyplot.plot(year, number, color='blue', label="actual value")  # 将实际值的折线设置为蓝色
    pyplot.plot(new_year[1:], s_pre_double[2:], color='red', label="double predicted value")  # 将二次指数平滑法计算的预测值的折线设置为红色
    pyplot.plot(new_year[1:], s_pre_triple[2:], color='green', label="triple predicted value")  # 将三次指数平滑法计算的预测值的折线设置为绿色
    pyplot.legend(loc='lower right')  # 显示图例的位置，这里为右下方
    pyplot.title('Projects')
    pyplot.xlabel('year')  # x轴标签
    pyplot.ylabel('number')  # y轴标签
    pyplot.xticks(new_year)  # 设置x轴的刻度线为new_year
    pyplot.show()
...
def main():
    ...
    initial_line = np.array(
        [0, 0, number[0]])  # 初始化，由于平滑指数是根据上一期的数值进行预测的，原始数据中的最早数据为1995，没有1994年的数据，这里定义1994年的数据和1995年数据相同
